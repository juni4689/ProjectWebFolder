...
- 이름에 해상도가 있으면 → 그 값이 **파일 크기와 아귀가 맞는지 검사한 뒤** 씁니다.
  맞지 않으면 이름을 버리고 파일 크기로 다시 추정합니다.
...
해상도를 어디서 얻었는지는 변환할 때마다 로그에 찍힙니다.

```
  cap.yuv -> cap_1920x1080_rgb24.raw
      1920x1080 nv12, 30프레임, 출력 178.0 MB  [해상도: 파일 이름]
```

옵션으로 직접 지정하면 파일 이름보다 우선하고, 검사도 건너뛰지 않습니다.

```bash
python yuv2raw.py ./in --size 1920x1080 --format nv12
```

### 해상도가 이상하게 나올 때: `--probe`

헤더가 없는 파일은 **같은 바이트열이 여러 해상도로 읽힙니다.**
예를 들어 2560x1440 4:2:0 3프레임과 2560x2160 4:2:0 2프레임은 바이트 수가 정확히 같습니다
(2160 = 1440 × 1.5). 파일 이름이 실제와 다르면 이런 오해가 조용히 통과할 수 있습니다.

`--probe` 는 변환하지 않고, 그 파일이 어떻게 읽힐 수 있는지 전부 보여줍니다.

```bash
python yuv2raw.py ./in --probe
```

```
cap_2560x2160.yuv
  파일 크기        : 16,588,800 바이트 (15.8 MB)
  이름에서 읽은 값 : 해상도 2560x2160, 포맷 없음
  지금 선택되는 값 : 2560x2160, i420, 2프레임  [파일 이름]
  가능한 해석 (프레임 수가 정확히 떨어지는 것만):
      해상도       프레임   포맷 후보
   >  2560x2160    2        i420 / nv12 / yv12 / nv21
      2560x1440    3        i420 / nv12 / yv12 / nv21
      1920x1080    4        i422 / yuyv / uyvy / nv16, gray_16
  원하는 해석을 --size WxH [--format FMT] 로 지정하세요.
```

`>` 가 지금 선택되는 해석입니다. 목록에서 원하는 줄을 찾아 그대로 지정하면 됩니다.

```bash
python yuv2raw.py ./in --size 2560x1440
```
...
python yuv2raw.py [경로 ...] [옵션]
  ...
  -j, --jobs N          동시에 변환할 파일 수 (기본: auto)
      ...
      --probe           가능한 해상도/포맷 해석을 모두 보여줌 (변환 안 함)
  -q, --quiet           로그 줄이기
...
| 해상도가 이상함 (특히 세로가 1.5배로 큼) | 파일 이름의 해상도가 실제와 다를 때 생깁니다. `--probe` 로 후보를 보고 `--size` 로 지정하세요. |
| `파일 이름의 해상도 ... 는 파일 크기와 맞지 않고` | 이름이 실제 내용과 다릅니다. `--probe` 로 확인 후 `--size` 지정. |
...
61개 테스트가 포맷 해석, 색 정확도, 두 백엔드의 바이트 단위 일치,
해상도 판별, 잘린 파일 거부, 원본 미변경 등을 확인합니다.
